...
`buy` 또는 `sell`이면 트레이더가 제안한 `candidate_symbols` 안의 같은 단일 종목을 사용한다. `yes_probability`, `news_score`, `top_candidate_symbols`는 앞선 세 보고서만 비교해 작성한다. 이 출력은 브로커 주문 승인이나 실행 명령이 아니다. 수량·가격·계좌 한도·장 상태·실시간 가격은 Python과 Toss API가 결정한다. `data_cutoff_utc`는 입력값을 그대로, `evidence_ids`는 입력 source evidence_id만 사용한다.
