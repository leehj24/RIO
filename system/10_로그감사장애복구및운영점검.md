...
flowchart TD
    ...
    A -->|대시보드에서 누가 무엇을 호출?| XL[log/api_logs.xlsx + dashboard_access.log]
...
| `dashboard_access.log` | Werkzeug 서버 시작·정상 접근·상태 코드. 콘솔 대신 회전 파일로 보존 | API 응답 본문과 전략 계산 내용 |
...
2. 이벤트 ID, 테마, 후보 수, 질문이 출력된다.
   대시보드의 반복 `127.0.0.1 ... GET ...` 줄은 콘솔이 아니라 `log/dashboard_access.log`에 기록된다.
   현금 맞춤 이벤트이면 그보다 먼저 `[cash-universe]`의 시장별 구매 가능·선택 수와 심볼 목록이 출력되고 `CASH_AFFORDABLE_UNIVERSE` 거래 로그가 추가된다.
...
4. `data/market_history.sqlite3`의 해당 심볼·간격 bar 수, `market_history_sync.last_error`, `history_exhausted`를 확인한다.
...
| 장기 시세 SQLite | 온라인 누적 | 정기 snapshot. 실행 중이면 SQLite backup 방식 사용 |
