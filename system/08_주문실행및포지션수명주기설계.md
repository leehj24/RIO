...
현금 맞춤 이벤트의 사전 구매 가능 판정은 주문 허가가 아니다. 분석 이후 다른 이벤트가 먼저 예산을 사용하거나 가격·환율이 움직일 수 있으므로, 실제 BUY 직전 남은 시장별 현금과 전체/종목당 상한으로 주문 금액을 다시 줄이고 최신 가격으로 주문 명세를 다시 만든다. 국내 1주 또는 미국 최소 금액을 더는 만족하지 못하면 `BUY_SKIPPED`로 끝난다.
