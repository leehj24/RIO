...
사용자가 심볼과 interval/count를 선택하면 `/api/candles`가 토스 캔들을 반환한다. 브라우저는 응답을 Chart.js 데이터로 변환해 가격 차트를 그린다. 차트에 보이는 값은 전략 연구 패킷의 정확한 동일 시점 snapshot과 다를 수 있으므로, 사후 감사에는 연구 패킷 hash와 거래 로그를 사용해야 한다.
...
- 직접 `python dashboard_server.py` 실행은 파일 내 라우트 선언 순서상 봇 전용 라우트 등록이 완전하지 않을 수 있어 `main.py` 통합 실행을 기준으로 한다.
