...
7. 활성 이벤트마다 후보를 만들고 시장/재무 자료를 수집한다.
8. 정량·AI 판단 후 종목별 주문 가능성을 평가한다.
9. 실행/차단 결과, 예측 레코드, LMSR 상태를 저장한다.
10. 한 주기가 끝나면 상태를 원자적으로 교체 저장한다.
...
| LLM 실패/한도 초과 | 불가 또는 보류 결과 | 신규 매수는 fail-closed, 결정적 청산은 계속 가능 |
...
- `state.json`, `trades.jsonl`, `data_cache/`의 쓰기 권한과 여유 공간을 확인한다.
