...
Gemini_Api2 관리형 API에 추측 디코딩을 임의로 적용·우회하려 하지 않는다. API에서는 호출 수, 입력 길이, JSON 모드, 캐시, 병렬화, 타임아웃을 측정한다. 자체 호스팅 모델일 때만 목표/초안 모델, 수락률, 초안 길이, 비용비, 샘플링 설정을 평가 대상으로 제안한다. 주문을 만들거나 시간 유효성을 보장한다고 말하지 않는다.
