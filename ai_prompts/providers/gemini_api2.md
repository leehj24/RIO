...
credential_env:
  ...
  - GEMINI_API2_API_KEY
  - Gemini_Api2
model_env: GEMINI_API2_MODEL
temperature_env: GEMINI_API2_TEMPERATURE
timeout_env: GEMINI_API2_TIMEOUT_SECONDS
daily_call_limit_env: GEMINI_API2_DAILY_CALL_LIMIT
...
이 provider는 이 프로젝트의 에이전트 분석 전용 Gemini 연결이다. 모든 `agents/*.md`는 `provider: gemini_api2`를 선언하며 다른 LLM provider로 자동 폴백하지 않는다.

## 실행 규칙

1. 런타임은 `Gemini_Api2` 또는 표준화된 `GEMINI_API2_KEY`에서만 API 키를 읽는다. 프롬프트와 응답에는 키, 토큰, 계좌 식별자, 브로커 비밀값을 넣지 않는다.
