...
client: NvidiaDeepSeekAgentClient
credential_env:
...
이 provider는 이 프로젝트의 에이전트 분석 전용 Gemini 연결이다. 모든 `agents/*.md`는 `provider: nvidia_deepseek`를 선언하며 다른 LLM provider로 자동 폴백하지 않는다.
